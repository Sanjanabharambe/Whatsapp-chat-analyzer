    ...
    try:
        ...
        if st.sidebar.button("Show Analysis"):
           ...
           # Sentiment Analysis
            ...
            df = functions.sentiment_analysis_vader(df)
            ...
            st.write("Sentiment Analysis Sample:")
            st.dataframe(df[['User', 'Message', 'sentiment_score', 'sentiment_label']].head(10))
         ...
         # Show Response Time Analysis Button
        # if st.sidebar.button("Show Response Time Analysis"):
        #         # Calculate response time if the button is pressed
        #     df = functions.calculate_response_time(df)

        #     st.title("User Interaction Patterns (Response Time Analysis)")
                
        #         # Bar Chart: Average response time by user
        #     st.subheader("Average Response Time by User")
        #     functions.plot_average_response_time_by_user(df)
                
        #         # Heatmap: Response time between users
        #     st.subheader("Response Time Between Users")
        #     functions.plot_response_time_between_users(df)
